...
---
```{r,echo=FALSE}
#muutuja defineerimine - sisesta toimeaine nimetus
ravim="agomelatiin"
ravim_omastavas="agomelatiini"
```

Selle dokumendi eesmärk on analüüsida täiendavate riskivähendamise meetmete rakendamist `r ravim_omastavas` näitel.
...
2. Toimeaine kasutamise alapunktis saab defineerida: 
...
5. Andmete võtmiseks teisest kataloogist asendada: example_data_tables=>data_tables
...
toimeaine <- read_excel(path=file.path(folder, 'toimeaine.xlsx'))
...
atc_mk <- read_excel(path=file.path(folder, 'atc_maksakahjustus.xlsx'))
```
...
rp_mk <- read_excel(path=file.path(folder, 'retseptidpatsiendidstartdate.xlsx'))
```
...
analyysid_er <- read_excel(path=file.path(folder, 'analyysid_enne_ravi.xlsx'))
```
...
hk <- read_excel(path=file.path(folder, 'hkkood.xlsx')) 
...
hk_2018 <- read_excel(path=file.path(folder, 'hk_2018.xlsx')) 
```
...
dl <- read_excel(path=file.path(folder, 'dl.xlsx'))
...
dl_2018 <- read_excel(path=file.path(folder, 'dl_2018.xlsx')) 
```

```{r}
#Vastavat toimeainet sisaldavate ravimitega ravi alustanud patsiendid aastate lõikes ja retseptid.
pra <- read_excel(path=file.path(folder, 'patsiendid_retseptid_aastate_loikes.xlsx'))
```

```{r}
#Patsiendid ja diagnoosid ravi alustamisel.
diagnoos <- read_excel(path=file.path(folder, 'patsient_diagnoos.xlsx')) 
```

```{r}
#Vastavat toimeainet sisaldavate ravimite kasutajate sugu ja vanus ravimi kasutama hakkamisel.
sugu_vanus <- read_excel(path=file.path(folder, 'sugu_vanus.xlsx'))
```
...
analyysid <- read_excel(path=file.path(folder, 'analyys_kpv.xlsx'))
```

```{r}
#Maksahaigus diagnoositud enne ravimi kasutama hakkamist, diagnoosid K70–K76 ja R74.0
mher2 <- read_excel(path=file.path(folder, 'mh_enne_ravi.xlsx'))
...
armms_tht <- read_excel(path=file.path(folder, 'ravimid_armms_tht.xlsx'))
```


```{r}
#andmed RHK-10
rhk_kood <- read_excel(path=file.path(folder, 'rhk_kood.xlsx'))
```
...
toimeaine$duration30 <- cut(toimeaine$duration, breaks = c(0,30,60, 90, 120, 150, 180, 210, 240, 270,300,330,
                                                   360,390,Inf),
...
ggplot(toimeaine, aes(x=duration30))+
...
```{r}
#saab defineerida uuritavad analüüside tegemise vahemikud vastavalt vajadusele
...
```{r}
#edasiseks analüüsiks uus dataframe, tingimusega, et tekstilist muutujat ei muudeta faktormuutujaks 
...
#andmete esitamine tabelina
#kogu tabeli nägemiseks tuleb andmed võtta dataframe'st diagnoos3t
diagnoos4t%>%
...
#Vähemalt ühe analüüsi saanud patsientide ja nende osakaalu visualiseerimine joonisel sõltuvalt analüüsi tegemise ajast
...
```{r,fig.cap = "Patsientide arv sõltuvalt ravi kestusest ja osakaal kohorti kuulunud patsientidest."}
...
```{r,fig.cap = "Maksafunktsiooni analüüside tegemine patsientidele sõltuvalt nende ravi kestusest ja täiendavates riskivähendamise meetmetes toodud nõuetest Alluvial-tüüpi diagrammil."}
...
```{r,message = FALSE,warning = FALSE,fig.cap = "Patsiendid, kelle ravi kestis üle 180 päeva ja nendele tehtud analüüside jaotus analüüside tegemise vajaduse perioodide lõikes."}
...
#lisamuutuja - patsientide arv, kelle ravi kestus >180 päeva
pat180 = sum(alus8$n)
...
```{r,results='hide',table.placement = "H"}
#lisamuutuja patsientide arv, kelle ravi kestus 31-180 päeva
...
#genereerib tabeli 
koik_analt%>%
  kbl(caption = "Patsientide arv, kellele tehti kõik maksafunktsiooni mõõtmise analüüsid vastavalt täiendavates
riskivähendamise meetmetes toodud nõuetele sõltuvalt patsientide ravi kestusest ja osakaal vastavas grupis.") %>%
  kable_classic("striped",full_width = F, html_font = "Times New Roman")%>%
  row_spec(0, bold = T)
```
...
#filter kõikidest patsientidest, kellele tehti kõik analüüsid
alus%>%
  filter(Enne_ravi==1, p1_30==1, p31_60==1, p61_120==1, p121_210==1)%>%
  count(subject_id)
...
Patsientide arv ja osakaal, kellel oli maksahaigus diagnoositud enne ravi:
```{r}
...
```{r,fig.cap = "Enne vastava toimeainega ravi alustamist maksahaiguse diagnoosiga patsientide arv RHK-10 diagnoosikoodide lõikes ja osakaal eelneva maksahaigusega patsientide seas."}
...
#lisab uue veeru, kuhu puuduvate väärtuste asemel paneb 40 ja 32, olenevalt patsiendi soost
asat2 <- asat2%>%
...
#üle piirväärtuse aga alla 3X piirväärtuse, üle 3X piirväärtuse, tulemus puudub
p2_alat <- alat3 %>%
  group_by(sugu)%>%
...
#genereerib tabeli 
alat_asat_t%>%
  kbl(caption= "Ravi ajal ja 30 päeva enne ravi tehtud ALAT ja ASAT laborianalüüside tulemused piirväärtuse järgi.") %>%
  kable_classic("striped",full_width = F, html_font = "Times New Roman")%>%
  row_spec(0, bold = T)
```
...
```{r,fig.cap = "Patsientidele ravi ajal ja 30 päeva enne ravi tehtud ALAT ja ASAT laborianalüüside tulemuste jaotus protsentuaalselt piirväärtuste järgi.",fig.pos= "h", warning=FALSE,message=FALSE}
...
#head(alat_asat_suurem3xret)
```

Patsientide arv, kelle analüüsi tulemus ületas 3-kordset piirväärtust ja kellele väljastati vastava toimeainega
retsepte ka peale maksatransaminaaside aktiivsuse tõusu:

```{r}
#kui palju patsiente, kellel analüüs üle 3-kordse piirväärtuse ja väljastati retsepte
n_distinct(alat_asat_suurem3xret$subject_id)
```
...
Patsientide arv, kelle analüüsi tulemus ületas 3-kordset piirväärtust ja kellele tehti kordusanalüüs 2 päeva
jooksul:
...
#kõrgem piirväärtusest, kuid võrdne või madalam 3-kordsest piirväärtusest
...
Patsientide arv, kelle analüüsi tulemus ületas piirväärtust, aga oli võrdne või madalam 3-kordsest piirväärtusest
ja kellele tehti kordusanalüüs 2 päeva jooksul:

```{r}
#väljastab patsientide arvu, kellele 'value > referents & value <= referents3' korral
#tehti kordusanalüüs 2 päeva jooksul
me <- alat_asat_suurem2xkpv%>%
summarize(Analyys2p = sum((diff_next > 0 & diff_next <= 2), na.rm = TRUE))%>%
filter(Analyys2p>0)%>%
distinct(subject_id)%>%
count()
